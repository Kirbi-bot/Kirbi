...
function checkMessageForCommand(msg, isEdit) {
	...
	if (msg.content.startsWith(Kirbi.Config.commandPrefix)) {
		...
		if (msg.isMentioned(Kirbi.Discord.user)) {
			try {
				...
				suffix = msg.content.substring(Kirbi.Discord.user.mention().length + cmdTxt.length + Kirbi.Config.commandPrefix.length + 1);
			} catch (err) {
	...
	} else if (Kirbi.Config.elizaEnabled === true && msg.isMentioned(Kirbi.Discord.user)) {
