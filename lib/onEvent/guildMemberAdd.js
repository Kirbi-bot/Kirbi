...
GReYBot.on('guildMemberAdd', member => {
    const channel = GReYBot.channels.find('name', (GReYBotConfig.antiraidChannelName || 'test'));
    ...
    recentMembers.push(member);
    member.send(`Welcome, ${member.user.username}, to the GReY Community!\nBe sure to read our <#${GReYBotConfig.welcomeChannel}> channel for an overview of our rules and features.`);
