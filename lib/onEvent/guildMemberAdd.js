...
GReYBot.watchingGuilds = {};
const raidProtectionSeconds = 10;
const raidProtectionUserLimit = 4;

GReYBot.Discord.on('guildMemberAdd', member => {
    const guildId = member.guild.id;
    const guild = GReYBot.Discord.guilds.find('id', guildId);
    
    // Add the server to the list of watched guild
    if (!GReYBot.watchingGuilds[guildId]) {
        GReYBot.watchingGuilds[guildId] = {
            channel: guild ? guild.defaultChannel : null,
            seconds: raidProtectionSeconds,
            limit: raidProtectionUserLimit,
            recentMembers: [],
            kicking: false
        };
    }
    ...
    const guildSettings = GReYBot.watchingGuilds[guildId];
    ...
    if(!guildSettings.kicking && member.guild.id === GReYBot.Config.welcomeGuild) {
